...
const RemoveGroups = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/users/${userId}/roles`, {
        data: { groups: groupsArray },
        headers: { "Content-Type": "application/json" },
      });
      setResponseMessage(`Groupes retirés avec succès : ${response.data.groups.join(", ")}`);
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ marginBottom: "1rem" }}>Retirer des groupes</h2>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        ...
        <div>
          <label>IDs des groupes à retirer (séparés par des virgules) :</label><br />
          <input
            type="text"
            value={groupIds}
